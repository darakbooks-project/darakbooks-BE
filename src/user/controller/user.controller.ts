...
import {
  ...
  unahtorizeddDTO,
  userNotfoundDTO,
} from 'src/dto/LoginResponse.dto';
...
export class UserController {
  ...
  @ApiOperation({
    summary: 'kakao로 소셜 로그인(회원가입) | kakao social login',
  })
  @ApiResponse({
    status: 200,
    description: '로그인 성공',
  ...
  async login(
    ...
    const { accessToken, refreshToken }: any = await this.authService.login(
      req.user,
    );
  ...
  @ApiTags('Authentication')
  @ApiOperation({
    summary: 'access token 만료시 refresh token을 이용해 재발급 | On access token timeout, reissuance with refresh token',
  })
  ...
  @ApiTags('my page')
  @ApiOperation({
    summary: '사용자 자신의 프로필을 요청할 때 사용 | When user is request own profile',
    description:
      '자신의 마이서재페이지의 프로필을 렌더링하는데 필요한 data 요청 | Calling data need to render my bookshelf profile',
  })
  ...
  @ApiTags('my page')
  @ApiOperation({
    summary: '자신의 프로필이 아닌 타인의 프로필을 요청할 때 사용 | When request different user profile',
    description:
      '타인의 프로필, 책장을 클릭했을 때 마이서재페이지의 프로필을 렌더링하는데 필요한 data 요청 | Calling data needed to render other profiles',
  })
  @ApiResponse({ status: 200, type: profileResDTO })
  @ApiParam({
    name: 'ownerId',
    type: 'string',
    description: '보고 싶은 profile 주인의 id',
  })
  @ApiUnauthorizedResponse({
    status: 401,
    type: unahtorizeddDTO,
  ...
  @ApiBearerAuth()
  @ApiTags('my page')
  @ApiOperation({ summary: 'profile 사진 등록 | Register profile image' })
  ...
  @ApiBearerAuth()
  @ApiTags('my page')
  @ApiOperation({ summary: '프로필 수정(수정하고 싶은 data만 전달하면 됨.) | Edit Profile' })
  @ApiResponse({ status: 200, type: profileResDTO })
